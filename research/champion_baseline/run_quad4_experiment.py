"""70-game experiment: 4th-quadrant unlock variant vs untouched champion.

Single change: BUY_LAND inserted at tick 290 (day 12, hour 2).
Farmer/hand movement paths are identical to the champion.

Test matrix (same as the plan-window experiment):
1. Head-to-head, variant vs baseline, both seat orders, 10 seeds -> 20 games
2. Variant self-play, 10 seeds -> 10 games
3. Baseline self-play, 10 seeds -> 10 games (paired reference)
...
variant = load("variant_quad4_main", "variant_quad4_unlock_main.py")
...
with open("experiment_quad4_results.json", "w") as f:
