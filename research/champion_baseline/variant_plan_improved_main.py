...
def plan(obs, hires=11, exponent=1.0, watering=True, sell='immediate', forecast=0):
    ...
    if hour<2:
        ...
        orders+=[['HIRE'] for _ in range(min(need,10))]
    sells=[['SELL',k,n] for k,n in stock.items() if k in PRODUCTS and n>0 and n*prices.get(k,0)>=5]
    sells.sort(key=lambda s:-s[2]*prices.get(s[1],0))
    orders+=sells[:10-len(orders)]
